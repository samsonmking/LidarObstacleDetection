...
std::vector<int> proximity(const std::vector<std::vector<float>>& points, int index, KdTree* tree, float distanceTol, std::unordered_set<int>& seen) {
  std::vector<int> cluster;
  seen.insert(index);
  cluster.push_back(index);

  auto nearbyPoints = tree->search(points[index], distanceTol);
  for (auto nearbyIndex : nearbyPoints) {
    if (seen.count(nearbyIndex) == 0) {
      cluster.push_back(nearbyIndex);
      auto nextCluster = proximity(points, nearbyIndex, tree, distanceTol, seen);
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol) {
  ...
  for (int i = 0; i < points.size(); i++) {
    if (seen.count(i) == 0) {
      clusters.push_back(proximity(points, i, tree, distanceTol, seen));
    }
  }
  return clusters;
}
...
int main() {
  ...
  for (std::vector<int> cluster : clusters) {
    ...
    for (int indice : cluster) clusterCloud->points.push_back(pcl::PointXYZ(points[indice][0], points[indice][1], 0));
